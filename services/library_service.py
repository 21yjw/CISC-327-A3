...
def return_book_by_patron(patron_id: str, book_id: int) -> Tuple[bool, str]:
    ...
    if(not book):
        return False, "Could not find borrowed book"
        
    return True, f'Successfully returned "{book["title"]} on {datetime.now()}.'
...
def calculate_late_fee_for_book(patron_id: str, book_id: int) -> Dict:
    ...
    if not patron_id or not patron_id.isdigit() or len(patron_id) != 6:
        return {'status':"Invalid patron ID. Must be exactly 6 digits."}
    ...
    if(not book):
        return {'status':"Could not find borrowed book"}
    ...
    fee = min(15, 
        max(0, min(overdueDays, 7)) * 0.50 +
        max(0, overdueDays - 7) * 1.00
    )
    
    return {
        'fee_amount': max(0, fee),
        'days_overdue': max(0, overdueDays),
        'status': 'success'
    }
...
def search_books_in_catalog(search_term: str, search_type: str) -> List[Dict]:
    ...
    if(search_type == "isbn"):
        ...
        books = get_all_books()
        return [item for item in books if (search_term.lower() in item['author'].lower())]
    elif(search_type == "title"):
        books = get_all_books()
        return [item for item in books if (search_term.lower() in item['title'].lower())]
...
def get_patron_status_report(patron_id: str) -> Dict:
    ...
    dueFees = 0;
    for b in borrowed:
        dueFees += calculate_late_fee_for_book(patron_id, b['book_id'])['fee_amount']
    borrowedAmt = get_patron_borrow_count(patron_id);
    ...
    for record in records:
        history.append({
            ...
            'return_date': None if record['return_date'] == None else datetime.fromisoformat(record['return_date']),
